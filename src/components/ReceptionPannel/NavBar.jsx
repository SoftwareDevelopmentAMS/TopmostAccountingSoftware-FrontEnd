/* eslint-disable react/prop-types */
import Profile from "../../assets/DashBoard/NavBar/Account.png";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import ProfileInfo from "./ProfileInfo";
import { faBars, faTimes } from "@fortawesome/free-solid-svg-icons";
const NavBar = ({ handleToggleSidebar, isSidebarOpen }) => {
  return (
    <div
      className={`  h-[4.5rem] text-ReceptionNavBarText  border  bg-ReceptionNavBar w-full relative `}
    >
      <div className="flex h-[4.5rem] w-full items-center   ">
        <FontAwesomeIcon
          icon={isSidebarOpen ? faTimes : faBars}
          onClick={handleToggleSidebar}
          className="w-10 hover:scale-[2] scale-150"
        />
        <div className="flex w-full items-center justify-end xs1:pr-10 ">
          <div className="xs1:w-60   rounded-lg overflow-hidden  h-10   relative">
            <input
              type="text"
              className="outline-none bg-[#fff] placeholder:text-xl placeholder:text-[#C6C6C6] placeholder:opacity-50 w-full px-5 py-2"
              placeholder="Search"
            />
            <img
              alt="svgImg"
              className="opacity-30  cursor-pointer  absolute top-1.5 right-0 xs1:right-5"
              src="data:image/svg+xml;base64,"
            />
          </div>

          <div className="pl-5 ">
            <ProfileInfo
              Profile={Profile}
              NameColor={" text-ReceptionNavBarText "}
              // Name={"Reception User-1"}
            />
          </div>
        </div>
      </div>
    </div>
  );
};

export default NavBar;
